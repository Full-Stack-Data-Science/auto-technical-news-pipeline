...
LOG_DIR="$WORK_DIR/logs/linkedin/post_publish"
LOG_FILE="$LOG_DIR/linkedin-publisher-$(date '+%Y-%m-%d_%H-%M-%S').log"
...
log "=========================================="
log "LinkedIn trending post publishing job started"
...
# ===== RUN JOBS =====
log "Running LinkedIn post publishing..."
START_TIME=$(date +%s)

"$PYTHON_BIN" "$WORK_DIR/service/linkedin_trending_posts_publish.py" \
    >> "$LOG_FILE" 2>&1
...
if [ "$SCRAPER_EXIT_CODE" -eq 0 ]; then
    ...
    log "ERROR: Publisher failed with exit code $SCRAPER_EXIT_CODE"
...
find "$LOG_DIR" -name "linkedin-publisher-*.log" -mtime +7 -delete
