...
LOCAL_OUTPUT_FILE = "linkedin_followers.json"

# Cloud (ADLS) remote path
CLOUD_REMOTE_PATH = "linkedin/relationships/followers/linkedin_followers.json"

# Define default followers to scrape
TOP_N_PER_INFLUENCER = 5


def parse_args(argv: list[str] | None = None) -> argparse.Namespace:
    parser = argparse.ArgumentParser(
        description="Scrape LinkedIn followers for influencers.",
...
def expand_from_existing_followers(
    base_influencers: List[str],
    top_n: int = TOP_N_PER_INFLUENCER,
) -> List[str]:
    """
    If a previously scraped followers JSON exists, pick the top-N followers
    for each influencer and merge them into the scrape list.

    Returns:
        Expanded list of influencer slugs (base + discovered followers),
        preserving original order with new entries appended.
    """
    local_path = os.path.join(LOCAL_OUTPUT_DIR, LOCAL_OUTPUT_FILE)
    if not os.path.isfile(local_path):
        logger.info(f"No existing followers file at {local_path} — scraping base influencers only")
    ...
    try:
        ...
        logger.warning(f"Could not read existing followers file: {e}")
        return base_influencers
    ...
    if extra_slugs:
        logger.info(
            f"Expanded scrape list with {len(extra_slugs)} followers "
            f"(top {top_n} per influencer) from {local_path}"
        )
    else:
        logger.info("Existing followers file found but no new slugs to add")
...
def scrape_followers(
    ...
    """
    Scrape followers for LinkedIn influencers.
    ...
    """
    logger.info("=" * 50)
    logger.info("Starting LinkedIn Followers Scraper")
    logger.info("=" * 50)
    ...
    with LinkedInConnectionsScraper(parser=parser) as scraper:
        # Login
        ...
        scraper.parser.login(email, password)
        logger.info("Successfully logged in")

        all_follower_results = []
        ...
        for idx, profile_url in enumerate(influencer_urls, 1):
            ...
            try:
                ...
                logger.info(f"Scraped {len(followers)} followers for {influencer_name}")
                all_follower_results.append(result)
        ...
        if all_follower_results:
            ...
            logger.info(f"Followers saved locally to {local_path}")
            total_followers = sum(len(r.get("followers", [])) for r in all_follower_results)
            logger.info(f"Total influencers with followers: {len(all_follower_results)}")
            logger.info(f"  Total followers scraped: {total_followers}")
            ...
            logger.warning("No follower results to save.")

        logger.info("=" * 50)
        logger.info("Followers Scraping Completed Successfully")
        logger.info("=" * 50)
...
def main(argv: list[str] | None = None) -> int:
    ...
    logger.info("=" * 50)
    logger.info("LinkedIn Followers Scraper")
    logger.info("=" * 50)
    ...
    influencers = expand_from_existing_followers(base_influencers)
    logger.info(f"Total profiles to scrape: {len(influencers)} "
                f"({len(base_influencers)} base + {len(influencers) - len(base_influencers)} from existing followers)")
