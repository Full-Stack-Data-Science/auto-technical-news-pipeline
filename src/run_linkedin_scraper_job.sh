...
LOG_DIR="$SRC_DIR/logs/linkedin/post_scraping"
LOG_FILE="$LOG_DIR/linkedin-scraper-$(date '+%Y-%m-%d_%H-%M-%S').log"
...
log "=========================================="
log "LinkedIn scraper job started"
...
# ===== RUN SCRAPER =====
log "Running LinkedIn scraper (headless mode)..."
if [ -n "$NO_UPLOAD" ]; then
    log "No-upload mode: scraped data will be saved locally only"
fi
START_TIME=$(date +%s)

# Enable headless mode for VM environment
log "Forcing headless Chrome via xvfb-run"
export RUNNING_IN_CRON=1
xvfb-run -a "$VENV_PYTHON" "$SRC_DIR/service/linkedin_scraper_execution.py" \
...
find "$LOG_DIR" -name "linkedin-scraper-*.log" -mtime +7 -delete
