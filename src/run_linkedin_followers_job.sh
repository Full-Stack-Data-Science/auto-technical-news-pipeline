...
LOG_DIR="$SRC_DIR/logs/linkedin/followers_scraping"
LOG_FILE="$LOG_DIR/linkedin-followers-$(date '+%Y-%m-%d_%H-%M-%S').log"

# Default page threshold
PAGE_THRESHOLD="${PAGE_THRESHOLD:-3}"
...
log "=========================================="
log "LinkedIn followers scraper job started"
log "Working directory: $WORK_DIR"
log "Source directory: $SRC_DIR"
log "Log file: $LOG_FILE"
log "Page threshold: $PAGE_THRESHOLD"
if [ -n "$NO_UPLOAD" ]; then
...
# ===== RUN FOLLOWERS SCRAPER =====
log "Running LinkedIn followers scraper (headless mode)..."
START_TIME=$(date +%s)

log "Forcing headless Chrome via xvfb-run"
export RUNNING_IN_CRON=1
xvfb-run -a "$VENV_PYTHON" "$SRC_DIR/service/linkedin_followers_scraper_execution.py" \
    --data "$WORK_DIR/data/influencer/linkedin_influencer.json" \
    --page-threshold "$PAGE_THRESHOLD" \
    $NO_UPLOAD \
    >> "$LOG_FILE" 2>&1
...
if [ "$SCRAPER_EXIT_CODE" -eq 0 ]; then
    log "Followers scraper completed successfully"
else
    log "ERROR: Followers scraper failed with exit code $SCRAPER_EXIT_CODE"
...
find "$LOG_DIR" -name "linkedin-followers-*.log" -mtime +7 -delete
